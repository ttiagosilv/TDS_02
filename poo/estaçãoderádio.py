...
class RadioFM:
    ...
    def mudar_frequencia(self, nova_frequencia=0):
        ...
        if nova_frequencia != 0:
            ...
            if nova_frequencia in self.__estações:
                ...
                self.__frequencia_atual = nova_frequencia
                self.__estação_atual = "Estação inexistente"
                print(f"Frequência inválida. Estação atual: {self.__estação_atual}")
        else:
    ...
    def get_frequencia_atual(self):
        return self.__frequencia_atual
    
    def get_estacao_atual(self):
...
# Criando um objeto do rádio
radio = RadioFM(vol_max=10, estações=estações)

# Habilitando a antena
radio.habilitar_antena()

# Ligando o rádio
radio.ligar()

# Aumentando o volume
radio.aumentar_volume(3)

# Mudando para uma estação específica
radio.mudar_frequencia(91.5)

# Mudando para a próxima estação
radio.mudar_frequencia()

# Diminuindo o volume
radio.diminuir_volume(2)

# Desligando o rádio
radio.desligar()

# Teste do que acontece quando o rádio está desligado
radio.aumentar_volume(1)